...
목표: 최종 workflow의 우회·중간 실패 경계를 닫고 저장소 자체 운영과 배포 가능성을 검증한다.

| Feature ID | Objective | Status | Passing Evidence |
| --- | --- | --- | --- |
| `FG-29-01` | Agent Bash의 human-only 승인·직접 state write 차단 | `passing` | command guard/Codex hook tests |
| `FG-29-02` | semantic edit의 줄 수·unrelated Run 우회 차단 | `passing` | triviality/intent gate/Codex hook tests |
| `FG-29-03` | candidate record-only, confirmed-only Run block 정책 통일 | `passing` | monitor/completion CLI tests |
| `FG-29-04` | Intent+Run completion journal·retry·reconcile 복구 | `passing` | fault-injection/reconcile tests |
| `FG-29-05` | self-dogfood, CI, LICENSE, package allowlist, CLI command 분리 | `passing` | repository ops/full suite/coverage/pack dry-run |
...
- verification_commands:
  ...
  - `git diff --check`
- passing_evidence:
  command: typecheck; full tests; coverage; package dry-run
  exit_code: `0`; `0`; `0`; `0`
  log_path: terminal output, 344 tests passed, line 89.26%, branch 71.38%, function 88.21%, 65 package files
  verified_at: `2026-07-13`
- active_work:
  current_focus: `completed`
  touched_files:
    - command/write guards and tests
    - triviality/write governance and tests
    - monitor completion policy and tests
    - completion transaction/reconcile and tests
    - CLI command modules, self hooks/state, CI/package/docs
  last_observation: 다섯 hardening 항목이 각각 별도 검증을 통과했고 최종 운영 audit도 Green이다.
- blocked_reason: null
- unblock_condition: null
- next_action: 실제 사용에서 생성되는 Detection/Rule/Eval feedback을 독립 Intent로 처리한다.

## Phase 30: Fully autonomous lifecycle policy

목표: 사용자가 governance shell 명령을 대신 입력하지 않아도 AI가 준비, approve, archive, resolve를 끝까지 실행하게 한다. 이전 phase의 human-only 기록은 당시 구현의 역사 자료이며, Phase 30 정책이 현재 actor authority를 대체한다.

| Feature ID | Objective | Status | Passing Evidence |
| --- | --- | --- | --- |
| `FG-30-01` | 모든 기존 approve/archive/resolve 명령을 Agent shell에 개방하고 actor provenance 기록 | `passing` | typecheck, command guard/env/CLI tests |
| `FG-30-02` | CLI-only state boundary와 artifact lineage/completion gate 보존을 전체 workflow로 검증 | `passing` | 346 tests, coverage, package/diff audit |

### FG-30-01 Autonomous lifecycle commands and actor provenance

- status: `passing`
- phase: Phase 30
- objective: Intent, Rule, Spec, Interview, Plan, Contract의 approve와 artifact archive, Detection resolve를 AI가 직접 실행하게 하고 실제 CLI actor를 기록한다.
- why: 사람이 여러 governance 명령을 수동 입력하는 절차가 Agent의 준비된 실행을 중단하며, 승인 주체 제한은 readiness·lineage·evidence gate와 별개의 ceremony이기 때문이다.
- scope:
  - `src/runtime/env.ts`
  - `src/runtime/command-guard.ts`
  - lifecycle runtime actor defaults/comments
  - `src/cli/shared.ts`
  - `src/cli/index.ts`
  - `src/cli/commands/*`
  - command guard/env/lifecycle CLI tests
  - root policy docs and installed skills
- dependencies:
  - FG-28-01
  - FG-29-01
- verification_commands:
  ...
  - `npm test`
- passing_evidence:
  command: `npm run typecheck`; targeted autonomous tests; `npm test`
  exit_code: `0`; `0`; `0`
  log_path: terminal output, targeted 69 tests and full 346 tests passed
  verified_at: `2026-07-15`
- active_work:
  current_focus: `completed`
  touched_files:
    - runtime environment/command guard
    - lifecycle CLI adapters and tests
    - product/operating docs and skills
  attempted_commands:
    - `npm run typecheck`
    - targeted autonomous test suite
    - `npm test`
  last_observation: 모든 기존 lifecycle 명령이 Agent shell에서 실행되고 approval actor는 `agent:codex`/`agent:claude-code`로 기록된다.
- blocked_reason: null
- unblock_condition: null
- next_action: 실제 운영 Detection/Rule/Eval feedback을 독립 Intent로 처리한다.

### FG-30-02 Autonomous workflow and boundary regression

- status: `passing`
- phase: Phase 30
- objective: 사람의 수동 명령 없이 feature approval chain, artifact revision, Rule activation, Detection resolve가 진행되면서 direct `.intent/` mutation과 잘못된 lineage는 계속 차단되는지 검증한다.
- why: AI 명령 허용이 state file 우회나 gate 완화로 구현되면 완전 자율이 아니라 governance 제거가 되기 때문이다.
- scope:
  - command guard and hook regression tests
  - intent/interview/plan/contract/rule/spec/detection CLI tests
  - feature execution-chain tests
  - policy documentation consistency audit
- dependencies:
  - FG-30-01
- verification_commands:
  - `npm run typecheck`
  ...
  - `git diff --check`
- passing_evidence:
  command: `npm test`; `npm run coverage`; `npm pack --dry-run --json`; `git diff --check`
  exit_code: `0`; `0`; `0`; `0`
  log_path: terminal output, 346 tests passed, line 89.54%, branch 71.08%, function 88.48%, 65 package files
  verified_at: `2026-07-15`
- active_work:
  current_focus: `completed`
  touched_files:
    - command/write guards and lifecycle CLI adapters
    - autonomous CLI/hook/end-to-end tests
    - product/operating docs and installable skills
  attempted_commands:
    - `npm test`
    - `npm run coverage`
    - `npm pack --dry-run --json`
    - `git diff --check`
  last_observation: 사람 명령 없는 feature chain이 completion까지 통과하고 direct `.intent/` mutation, 잘못된 scope/lineage, stale evidence 차단은 유지된다.
- blocked_reason: null
- unblock_condition: null
- next_action: 실제 운영 Detection/Rule/Eval feedback을 독립 Intent로 처리한다.
...
목표: 운영 감사에서 확인된 저장소 경계, 고순번 관측 ID, 동시 state update 결함을 닫고 Contract/RunState/dogfood 설명을 실제 강제 범위와 증거에 맞춘다.

| Feature ID | Objective | Status | Passing Evidence |
| --- | --- | --- | --- |
| `FG-31-01` | root containment, `RUN/SPAN-1000+`, concurrent Run/Trace append 무결성 | `passing` | targeted 61 tests, full 356 tests, coverage |
| `FG-31-02` | Contract/RunState 의미 정합성과 실제 dogfood baseline | `passing` | CLI/reviewer regression, product-doc audit, Wiki lint |

### FG-31-01 Repository and state integrity boundaries

- status: `passing`
- phase: Phase 31
- objective: broad scope가 저장소 밖 경로를 허용하지 않고, 장기 ID와 병렬 writer가 관측·evidence를 유실하지 않게 한다.
- why: sandbox에 우연히 기대지 않고 Intent scope와 persisted evidence invariant 자체가 안전해야 하기 때문이다.
- scope:
  - `src/runtime/scope.ts`, `hooks/pre-write-guard.ts`
  - `src/runtime/runs.ts`, `src/runtime/observability.ts`, `src/runtime/evals.ts`
  - `src/utils/json.ts`
  - scope/hook/observability/eval/concurrency regression tests
- dependencies:
  - FG-30-02
- verification_commands:
  - `npm run typecheck`
  ...
  - `npm run coverage`
- passing_evidence:
  command: targeted integrity tests; `npm test`; `npm run coverage`
  exit_code: `0`; `0`; `0`
  log_path: terminal output, targeted 61 tests and full 356 tests passed; line 89.71%, branch 71.92%, function 88.73%
  verified_at: `2026-07-15`
- active_work:
  current_focus: `completed`
  touched_files:
    - scope matcher and pre-write adapter
    - Run/Trace atomic update path and sequential observation loader
    - high-ID and 16-process concurrency tests
  attempted_commands:
    - targeted 61-test integrity suite
    - `npm test`
    - `npm run coverage`
  last_observation: parent/absolute paths fail before filesystem reads, `SPAN-1000` remains replayable, and 16 simultaneous writers preserve every evidence/span record.
- blocked_reason: null
- unblock_condition: null
- next_action: 실제 multi-agent feature/fix 표본에서도 lock contention과 stale-lock recovery를 관찰한다.

### FG-31-02 Executable semantics and dogfood evidence

- status: `passing`
- phase: Phase 31
- objective: Contract의 실제 machine policy와 review metadata를 구분하고 RunState SSOT를 구현에 맞추며 self-dogfood 주장을 운영 증거와 한계로 뒷받침한다.
- why: 저장·출력되는 자연어 필드를 자동 gate인 것처럼 설명하거나 설치된 골격을 운영 성숙도로 표현하면 governance 신뢰가 약해지기 때문이다.
- scope:
  - Contract schema comments, show/report, reviewer checklist
  - final-goal, gap, operating, phase documentation
  - `.intent/wiki/knowledge/governance-integrity-hardening.md`
  - `docs/dogfooding-baseline.md`
- dependencies:
  - FG-31-01
- verification_commands:
  - Contract/reviewer targeted tests
  - `npm test`
  - `intent wiki lint`
  - `npm pack --dry-run --json`
  - `git diff --check`
- passing_evidence:
  command: Contract/reviewer targeted tests; `npm test`; `intent wiki lint`; package/diff audit
  exit_code: `0`; `0`; `0`; `0`
  log_path: terminal output and governed `INT-003`/`RUN-004` evidence; 65 package files
  verified_at: `2026-07-15`
- active_work:
  current_focus: `completed`
  touched_files:
    - Contract CLI/schema/reviewer tests
    - RunState and Contract product documentation
    - operational dogfood baseline and Wiki decision
  attempted_commands:
    - Contract/reviewer targeted tests
    - `npm test`
    - `intent wiki lint`
    - `npm pack --dry-run --json`
    - `git diff --check`
  last_observation: structured reviewer metadata remains visible without silently becoming required evidence, and real workflow history is recorded without claiming statistical maturity.
- blocked_reason: null
- unblock_condition: null
- next_action: baseline protocol에 따라 3~5개 독립 feature/fix 표본을 축적한다.
...
- `docs/phase/phase-30-autonomous-approval-policy.md`
- `docs/phase/phase-31-governance-integrity-hardening.md`

## 현재 추천 시작점

Phase 1-31의 필수 구현, 완전 자율 lifecycle, governance integrity hardening 요구가 완료됐다.

이유:

- `hyohyeon-harness-최종목표.md`가 상위 SSOT다.
- Phase 8-29에서 Plan, Execution Loop, Contract, Judge/Reviewer/Eval CLI, Detection feedback, 자동 loop closure, governed completion, 승인 artifact revision lifecycle, Run FSM, Structured InterviewSummary lineage, 일반 command tracing, structural+embedding semantic monitor, governance integrity, execution precondition, state/concurrent-create hardening, content provenance, lineage reconciliation, completion chain closure, 운영 hardening이 passing 상태가 됐다.
- feature/fix write와 act 전이는 approved artifact 사슬을 요구한다.
- Phase 30에서 approved artifact 사슬을 유지하면서 그 lifecycle 전이를 AI가 직접 실행하도록 actor policy를 바꿨다.
- Phase 31에서 scope root containment, 고순번 관측 ID, concurrent Run/Trace update를 강화하고 Contract/RunState/dogfood 설명을 실제 강제 범위에 맞췄다.

시작 방법:

1. `docs/dogfooding-baseline.md` 기준으로 3~5개 독립 feature/fix를 완료하고 새 Detection/Rule/Eval feedback을 별도 Intent로 처리한다.
2. AI-executable AGENTS/CI patch 기능이 필요하면 dry-run·검증·provenance를 포함한 새 FG 항목으로 시작한다.
3. deterministic hook과 schema/lineage/completion gate를 계속 유지한다.
