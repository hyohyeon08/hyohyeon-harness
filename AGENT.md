...
- 최종목표 기준 Phase 1-29 핵심 workflow와 운영 hardening 구현. Claude Code와 Codex 어댑터 지원. 344/344 테스트 통과.
...
│   ├── cli/shared.ts           # CLI context·공통 human/flag helper
...
npm test          # build → node --test tests/*.test.mjs (344)
...
- 필수 구현 gap은 없다. 저장소 자체도 `.intent/`와 상대경로 Claude/Codex hook으로 동일 게이트를 dogfood한다.
...
- AGENTS/CI 자동 patch 적용은 의도적으로 candidate/reflection 추적까지만 제공하고 사람이 선택하는 후속 기능으로 남긴다.
