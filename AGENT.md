...
| 4 | `.intent/`는 **CLI만** 쓴다. AI의 직접 Edit/Write는 가드가 차단한다. 승인 명령은 `CLAUDECODE=1`에서 거부된다(human-only). | `guard.ts`, `env.ts` |
...
- v1(게이트 레이어) + v2(지식 레이어) 완성. 75/75 테스트 통과.
...
│   │   ├── env.ts              # CLAUDECODE 승인 가드 (anti-cheat 2차)
...
├── .claude/settings.template.json  # {{INTENT_ROOT}} placeholder
...
npm test          # build → node --test tests/*.test.mjs (75)
...
```bash
intent setup --install-hooks   # .intent/ 생성 + .claude/settings.json 렌더·머지 + skills 복사
```
...
| `PreToolUse (Edit\|Write)` | `pre-write-guard.ts` | ①`.intent/` 보호 ②게이트 규칙 ③의도+스코프 게이트 | **예** | stdout JSON / stderr |
...
- `intent approve` / `intent rule approve` / `intent spec approve`는 `CLAUDECODE=1`(AI 셸)에서 거부된다 — 사람의 셸에서만 (`env.ts`). Bash 우회까지 차단.
...
5. AI가 `intent approve`를 셸에서 실행 → CLAUDECODE 가드 거부. 사람이 한다.
