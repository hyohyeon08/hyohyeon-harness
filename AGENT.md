...
현재 구현은 AI가 코드를 짜기 전 **의도(무엇·왜)를 선언·승인**받게 하고, Agent 실행 상태·검증 증거·관측 데이터·탐지 기록·지식·컨텍스트가 손실되지 않게 하는 개인 워크플로 하네스다. 세 레이어로 구성된다: **게이트 레이어**(의도·스코프·DoD·anti-cheat·contract), **실행 증거 레이어**(RunState·VerificationEvidence·Trace/Span·Detection), **지식 레이어**(위키·핸드오프·실패규칙·spec).
...
- 최종목표 기준 Phase 1-28 핵심 workflow와 hardening 구현. Claude Code와 Codex 어댑터 지원. 330/330 테스트 통과.
- TypeScript + zod + `node:test`. 외부 런타임 의존성 없음(zod만).
- 필수 구현 gap: 없음. Upstream hook 밖 shell은 `intent command` wrapper가 필요하고 AGENTS/CI patch는 의도적으로 human-selected candidate다.
...
│   ├── pre-command-guard.ts    # PreToolUse(Bash): 승인·직접 state write 차단
...
npm test          # build → node --test tests/*.test.mjs (330)
...
| `PreToolUse (Bash)` | `pre-command-guard.ts` | 승인 CLI와 직접 `.intent/` state write를 실행 전에 차단 | **예** | stdout JSON / stderr |
...
- `intent approve`와 artifact/rule approval·archive·detection resolve는 Agent Bash의 `PreToolUse`에서 차단되고, CLI도 AI 환경 표식을 다시 확인한다.
- 이 경계는 설치된 Claude/Codex hook과 CLI를 따르는 **지원 채널의 human-confirmation 경계**다. 같은 OS 사용자 권한으로 hook 밖에서 임의 프로세스를 실행하는 적대자를 격리하는 보안 샌드박스는 아니며, 그 위협에는 별도 OS 계정·sandbox·외부 승인 broker가 필요하다.
...
- Execution precondition: feature/fix write와 `contract -> act` 전이에 approved Contract를 요구한다.
- Broader state hardening: Run/Plan/Rule/Detection state 손상과 ID allocation concurrency를 강화한다.
- Evidence freshness/provenance: verification이 마지막 코드 revision 이후 생성됐는지 증명한다.
- Semantic similarity/LLM Judge 자동화: 구조 후보 중 애매한 항목에만 비용 제한 판정을 실행한다.
- AGENTS/CI 자동 patch 적용은 아직 의도적으로 candidate/reflection 추적까지만 제공한다.
