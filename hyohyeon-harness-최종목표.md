...
* 사용자가 목표를 위임한 뒤 준비·승인·archive·resolve 명령을 AI가 끝까지 수행하여 수동 shell 개입을 0으로 만든다.
...
Sprint Contract
  ↓
Autonomous Readiness / Lineage Transition
  ↓
RunState 생성
  ↓
Execution Loop
...
### 4.1 완전 자율 lifecycle 정책

사용자가 작업 목표를 위임한 뒤 governance lifecycle을 진행하기 위해 별도 shell 명령을 입력하는 단계는 없다. AI는 준비가 끝나면 `approve`, `archive`, `resolve`를 포함해 과거 사람 전용이었던 모든 `intent` CLI 명령을 스스로 실행하고 다음 phase로 진행한다.

`approved`는 사람의 보증이나 별도 권한 부여를 뜻하지 않는다. 해당 artifact가 실행 준비 조건을 충족했고, 같은 Intent/Run lineage에 고정되었으며, 이후 변경은 revision lifecycle을 거쳐야 한다는 기계적 readiness 상태다. `archived`와 detection `resolved`도 같은 방식으로 AI가 실행하는 lifecycle 전이다.

자율 실행은 추적 가능해야 한다. 승인 계열 전이는 `human`, `agent:codex`, `agent:claude-code`처럼 실제 CLI actor provenance와 시각을 state에 기록한다. Actor 종류에 따라 전이 권한을 달리하지 않으며, 같은 schema·lineage·phase precondition을 적용한다.

완전 자율 정책은 state 보호를 완화하지 않는다. `.intent/` 직접 Edit/Write, redirect, 임의 script mutation은 계속 차단하고, 모든 상태 전이는 schema validation을 거치는 `intent` CLI만 수행한다. 즉, 제거하는 것은 사람의 수동 승인 절차이지 gate, immutability, provenance, completion evidence가 아니다.

Phase 30 이전 문서에 남아 있는 human-only 설명은 당시 구현의 역사 기록이다. 현재 actor authority가 충돌할 때는 이 완전 자율 정책이 우선한다.
...
Interview Summary는 목표와 이유뿐 아니라 context, constraints, allowed/forbidden scope, success/failure criteria, verification, considered options, non-goals, assumptions, open questions를 schema-validated JSON으로 저장한다. AI가 readiness를 확인해 approved로 전이한 뒤 본문은 불변이며, 아직 존재하지 않았던 Intent/Spec/Plan/Run 참조만 기존 값을 교체하지 않는 append-only 방식으로 연결한다. 이 전이는 actor provenance와 시각을 남긴다.
...
Plan은 `draft -> approved -> archived` 생명주기를 가진다. AI는 Plan 준비가 끝나면 직접 approved로 전이하며 actor와 시각을 남긴다. Approved Plan의 실행 전략·스코프·검증·DoD는 수정할 수 없고, 변경이 필요하면 AI가 lifecycle 순서에 따라 archive한 뒤 새 draft revision을 만들고 다시 readiness를 확정한다.

Interview/Plan/Contract revision은 `revision` 번호와 `supersedes*Id`를 기록한다. Approved artifact는 AI가 CLI로 archive한 뒤에만 revision할 수 있다. Archive는 연결된 Run pointer를 비우고 `contract`/`plan`/`interview` phase로 되돌려 새 revision readiness 전 실행을 중단한다.
...
Sprint Contract도 readiness 전에는 draft다. AI가 lineage와 실행 준비 조건을 확인해 approved로 전이하며, actor provenance와 시각을 남긴다. Draft Contract는 실행 게이트나 completion policy를 바꾸지 않고, approved Contract만 allowed/forbidden scope와 required checks의 근거가 된다. Approved 내용은 직접 바꾸지 않고 AI가 archive/revise CLI를 실행해 새 Contract revision을 만든다.

Contract의 모든 필드가 같은 방식으로 실행되는 것은 아니다. `status`와 Run/Intent/revision lineage, `allowedScope`, `forbiddenScope`, `requiredChecks`는 hook·phase·completion evaluator가 읽는 기계 강제 정책이다. `architectureBoundaries`, `definitionOfDone`, `rubric`, `stopConditions`, `requiresUserDecision`는 reviewer checklist와 report에 노출되는 구조화된 리뷰 메타데이터이며 자동 completion gate가 아니다. 이 자연어 필드를 강제하려면 별도의 deterministic rule이나 required check로 명시적으로 compile해야 한다.
...
RunState를 통해 대화가 끊기거나 모델이 바뀌어도 작업을 이어갈 수 있다. Intent의 DoD와 Contract의 scope/test matrix/completion policy, Trace/Span과 Detection record는 각자의 schema-validated artifact에 저장하고 RunState는 그 lineage ID와 verification evidence를 보유한다.
...
* 시도 횟수가 `maxAttempts`에 도달하면 Run을 `blocked`로 바꾸고 `nextAction`에 exhaustion 이유를 기록한다.
...
Artifact readiness와 실제 실행은 같은 사슬이어야 한다. 연결된 Interview가 있으면 approved Interview만 plan phase의 근거가 되고, `plan -> contract`는 같은 Run/Intent의 approved Plan, `contract -> act`는 같은 Run/Intent의 approved Contract를 요구한다. AI가 각 approve/phase 명령을 직접 실행하더라도 이 precondition을 건너뛸 수 없다. feature/fix의 비사소 write는 Run이 `act` 또는 `verify`이고 approved Contract가 연결된 동안에만 허용한다.
...
`latest-runs.json`은 validated RunState에서 재구축 가능한 derived cache다. Cross-artifact recovery는 기존 lineage를 교체하지 않고 비어 있는 backlink만 채우며, 서로 다른 기존 값이 충돌하면 자동 적용하지 않고 Agent에게 fail-closed blocker로 보고한다.
...
Rule Candidate
  ↓
Agent / Judge Resolution
  ↓
Test / Lint / CI / Hook / Eval 반영
```

이렇게 하면 리뷰는 단순한 일회성 코멘트가 아니라, Harness를 강화하는 입력 데이터가 된다.
...
### Run 상태와 completion 판정

저장되는 `RunStatus`는 `active | blocked | passing | paused` 네 가지다.

* `active`: 현재 실행 초점인 Run
* `blocked`: 실행을 계속할 수 없는 이유가 `nextAction`과 필요하면 `notes`에 기록된 Run. Governed Run과 completion 책임은 유지된다.
* `passing`: 실행 결과의 통과 표지로 active 초점에서 제외된 Run. 이 status 자체는 completion authority가 아니다.
* `paused`: active 실행 초점에서 잠시 제외된 Run. 이 status도 completion 책임을 제거하지 않는다.

`complete`, `failed`, `unsafe`, `budget_exhausted`는 RunStatus 값이 아니다. Completion gate가 성공하면 terminal 상태는 `phase: done`, `status: passing`으로 기록된다. 검증 성공과 실패는 `VerificationEvidence.status`의 `passed | failed`로 저장하며, `missing`과 `stale`은 completion evaluator가 계산하는 차단 사유다.

현재 Run budget은 `maxAttempts`와 `attemptsUsed`만 저장한다. `attemptsUsed >= maxAttempts`이면 Run을 `blocked`로 바꾸고 `nextAction`에 attempt budget exhaustion 이유를 남긴다. 별도 wall-clock budget이나 `budget_exhausted` status는 없다. 권한·scope·unsafe 작업은 `unsafe` status를 만들지 않고 해당 guard가 작업을 거부하며 차단 이유를 반환한다.
...
구현 정책상 `candidate` detection은 Run을 자동 hard block하지 않는다. 같은 파일의 20-line bucket 반복 수정과 `edit -> failed command` 같은 tool sequence 반복은 후보로 저장하며, Agent 또는 판정자가 CLI로 `confirmed`를 기록한 뒤에만 blocked 전이를 만든다. Detection의 `resolve`도 AI가 직접 실행하고 결과·근거·시각을 보존한다. Detection에서 만든 regression eval은 source JSON을 자기 비교하지 않고 관련 span snapshot을 detector 입력으로 재생한다.
...
* `phase: done`인데 required verification evidence가 충족되지 않은 Run
* trace/span이 누락된 Run
...
* Wiki lint는 매번 실행하지 않고 Agent가 필요할 때 또는 주기적으로 실행한다.
* AGENTS.md/CI 변경은 처음에는 rule candidate로 생성한다. 적용 기능이 추가되면 사람 전용 gate가 아니라 AI가 실행 가능한 dry-run·검증 기반 CLI 전이로 만든다.
...
* wiki와 lifecycle 판단은 AI가 관리하되 actor provenance와 근거를 남기고 schema/evidence gate로 검증한다.
...
* governed Run이 없거나 required type별 최신 evidence가 missing/failed인 `phase: done` 상태는 허용하지 않는다.
...
사용자가 목표를 위임한 뒤 lifecycle 승인을 위해 다시 shell로 개입하지 않는다. Agent가 준비, approve, archive, resolve, phase 전이, 검증, 완료까지 CLI를 통해 자율 수행하고 Harness는 그 actor와 근거를 보존한다.
