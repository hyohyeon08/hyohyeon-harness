...
- **안 넣는다**: *차단해야 하는* 규칙은 위키가 아니라 **게이트 규칙**(`intent rule …`)에 둔다. Agent는 규칙 후보의 정확성·영향 범위·근거를 확인한 뒤 `intent rule approve <ruleId>`를 직접 실행한다. 위키 = 읽는 지식, 규칙 = 강제되는 것. 휘발성 잡담도 제외.
